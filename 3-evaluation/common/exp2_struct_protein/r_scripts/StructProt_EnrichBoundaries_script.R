...
chrs = as.character(substr(unique(Tads_chr$V1),4,nchar(unique(Tads_chr$V1))))
paste(chrs)
...
for (protein in proteins)
{
   ...
   for (chr in c(chrs))
   {
      ...
      cTads_chr = Tads_chr[Tads_chr$V1==paste0('chr',as.character(chr)),]

      Tad_edges <- unique(c(cTads_chr[,2], (cTads_chr[,3]+5000)))
      # print(Tad_edges)
...
pdf(paste0(OutFolder, "Enrich_StructProt_chr",paste(chrs,collapse="_"),"_res",resolution_kb,"kb.pdf"))
...
write.table(plot_df, file = paste0(OutFolder, "Enrich_StructProt_chr",paste(chrs,collapse="_"),"_res",resolution_kb,"kb.txt"), row.names = F, col.names = T, quote = F )
...
plot(x=c(1:as.integer(1000/res_step)), y=Allprots_enrichs[,"CTCF"], type = "l", col = color1, xlab = "Distance from TAD boundary", ylab = paste0("Average # of peaks / ", as.character(res_step)," kb"), xaxt='n', cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5,main=title,frame.plot = FALSE)
...
write.table(Final_Ratios_df, file = paste0( OutFolder, "StructProteins_chr",paste(chrs,collapse="_"),"_res",resolution_kb,"kb.txt" ), quote = FALSE, sep = "\t", row.names = FALSE)
