...
def main() -> None:
    ...
    stats_list = []
    for tool in m.TOOLS:
        match_num, total_num, fraction = m.calculate_ctcf_fraction(tool, m.DATA_DIR, m.CTCF_FILE, boundary="right")
        stats_list.append(
            {
                "tool": tool,
    ...
    out_csv = m.OUTPUT_DIR / "exp4_results.csv"
    stats_df.to_csv(out_csv, index=False)
    # backward-compatible filename
    stats_df.to_csv(m.OUTPUT_DIR / "exp8_results.csv", index=False)
    print(out_csv)
